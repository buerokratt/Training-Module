...
import MoveExampleModal from "./MoveExampleModal";
...
const IntentExamplesTable: FC<IntentExamplesTableProps> = ({intent, updateSelectedIntent, reFetchIntents}) => {
    ...
    const [selectedExample, setSelectedExacmple] = useState<string>('');
    ...
    const handleMoveExample = async (formData: MoveExampleForm) => {
        try {
            const intentsResponse = await queryClient.fetchQuery<IntentResponse>([
                `intents/by-id?intent=${formData.intent}`,
            ]);

            let examples = intentsResponse.response.examples;

            await addExamplesMutation.mutateAsync({
                intentName: formData.intent,
                intentExamples: examples,
                newExamples: formData.example.replace(/(\t|\n)+/g, ' ').trim(),
            });

            await exampleDeleteMutation.mutateAsync({
                intentName: intent.id,
                example: formData.example,
            });

        } catch (error) {
            console.error('Failed to move example: ', error)
        }
    ...
    const examplesColumns = useMemo(
        () => [
            ...
            columnHelper.display({
                header: '',
                cell: (props) =>
                    buildMoveExampleCell(
                        () => {
                            const currentExample = props.row.original.value;
                            setSelectedExacmple(currentExample);
                            setShowModal(true)
                        }
                    ),
                id: 'moveExample',
