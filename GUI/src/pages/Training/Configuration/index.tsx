...
const Configuration: FC = () => {
  ...
  const { data: configurationData } = useQuery({
    ...
    onSuccess: (data: any) => {
      reset(data.response);
      setInitialConfig(data.response);
  ...
  const configurationDataMutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries(['config/update']);
      toast.open({
        type: 'success',
    ...
    <>
              ...
              <FormSelect
                ...
                label={
                  <>
                    ...
                    >
                      <a href="#">
                        {t('training.configuration.moreFromKratid')}
                      </a>
                    </p>
                  </>
                }
                defaultValue={configurationData.response.recipe}
                onSelectionChange={(selection) => field.onChange(selection)}
                options={[
                  {
                    label: configurationData.response.recipe,
                    value: configurationData.response.recipe,
                  },
                ]}
              />
            )}
          ...
          <Controller
            ...
            render={({ field }) => (
              <FormSelect
                ...
                defaultValue={configurationData.response.language}
                onSelectionChange={(selection) => field.onChange(selection)}
      ...
      <Tabs.Root className="vertical-tabs" orientation="vertical">
        <Tabs.List
          ...
          aria-label={t('training.configuration.pipeline') || ''}
        >
          {configurationData.response.pipeline.map((comp, index) => (
            <Tabs.Trigger
              key={`${comp.name}-${index}`}
              value={`${comp.name}-${index}`}
      ...
      <Tabs.Root className="vertical-tabs" orientation="vertical">
        <Tabs.List
          className="vertical-tabs__list"
          aria-label={t('training.configuration.policies') || ''}
        >
          {configurationData.response.policies.map((policy, index) => (
            <Tabs.Trigger
